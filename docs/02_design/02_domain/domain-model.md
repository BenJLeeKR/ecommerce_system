...
- **AI가 임의로 판단하면 안 되는 정책**
  - 재고 예약 유효시간(TTL) 15분, 조건부 원자 갱신을 통한 동시성 제어 규칙이 승인되었으므로 이에 맞게 구현한다. 임의로 방식을 변경하지 않는다.
...
- **비즈니스 규칙**
  ...
  - 결제 반복 실패는 동일 주문에 대해 3회 누적까지만 허용하며, 3회 누적 후에는 재결제를 차단한다. 매 실패 시 재고는 해제되나, 복구 로직을 중복 수행하지 않는다.
- **AI가 임의로 판단하면 안 되는 정책**
...
3. **예약에는 유효시간(TTL)이 있어야 한다.** 고객이 결제 화면을 열어둔 채 방치하면 재고가 무한정 묶이므로, KST 기반으로 스케줄링되어 15분이 지나면 자동으로 해제되어야 한다.
...
4. **반복 실패 대응**: 동일 주문에 대해 결제 실패가 반복되는 경우, 3회 누적 시 완전 실패 처리 및 추가 재결제를 차단한다. 매 실패 시 예약 재고는 즉시 해제하며, 세 번째 실패 후 재고 복구를 중복해서 수행하지 않는다.
...
2. PG 콜백이 도착하면, 먼저 "이 멱등키로 이미 처리된 적이 있는가"를 확인한다.
   - 이미 처리된 경우: PG에는 정상 응답을 다시 보내되, 주문 확정·재고 확정·알림 발송 등 **실제 처리는 다시 실행하지 않는다.** 단, 이미 `FAILED` 처리된 후 성공 콜백이 도착하는 등 상태 전이와 맞지 않는 '역순·불일치 콜백'의 경우에는 단순 무시하지 않고 모니터링 알림 후 운영팀 수동 대사 처리로 연계한다 (민감정보는 로그에 남기지 않음).
...
- [ ] ➕ 확인 필요 항목 일괄 검토 (로그인 잠금 정책, 카테고리 삭제 정책, PREPARING 취소 정책)
