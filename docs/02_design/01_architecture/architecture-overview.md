...
- 공용 PostgreSQL 자체의 백업/장애 대응은 공용 인프라를 관리하는 주체의 책임 범위이므로, 이 프로젝트만의 백업 주기·복구 절차가 별도로 필요한지는 인프라 관리자와 별도로 확인이 필요하다. (➕ 확인 필요 항목으로 표시)
...
> ➕ 확인 필요: 공용 PostgreSQL의 백업 주기, 장애 시 복구 절차, 접속 계정/권한 발급 방식은 공용 인프라 관리자에게 별도로 확인하고, 확인된 내용을 이 문서에 추가한다.
...
- [ ] 결제 PG 연동 위치 및 흐름 승인
- [ ] 공용 PostgreSQL 백업/장애 대응 절차 확인 (인프라 관리자 확인 필요)
