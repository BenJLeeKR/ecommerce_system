...
- **Unique 제약**: `idempotency_key` (`domain-model.md` 8장 중복 콜백 처리의 핵심 장치), `pg_transaction_id` (MVP 단일 PG 기준 거래번호 유일성 보장)
...
1. **주문 생성 + 재고 예약은 하나의 트랜잭션으로 묶는다.** 둘 중 하나만 성공하는 상황(주문은 생성됐는데 재고는 예약 안 됨, 또는 그 반대)이 있으면 안 된다. 재고 만료는 `Asia/Seoul` 기준 1분 단위 스케줄러로 처리한다.
2. **재고 예약 시 동시성 제어가 필수다.** 여러 주문이 동시에 들어와도 가용재고(=`total_quantity - reserved_quantity`) 이하로만 예약이 성립해야 한다. "가용재고를 먼저 조회하고 나중에 차감"하는 방식(조회-후-갱신)은 동시 요청 시 오버셀을 유발할 수 있으므로, 승인된 원칙에 따라 **조건을 만족해야만 갱신되는 조건부 원자적 연산**(`UPDATE ... WHERE quantity >= X`)을 사용하여 처리한다.
3. **외부 PG 호출처럼 느리고 실패할 수 있는 작업은 DB 트랜잭션 안에 넣지 않는다.** PG 호출은 트랜잭션 밖에서 수행하고, 그 결과(성공/실패)만 트랜잭션으로 반영한다. (트랜잭션이 오래 걸리면 락이 오래 유지되어 다른 주문 처리가 지연될 수 있음)
4. **결제 승인 콜백 처리(결제 상태 갱신), 주문 상태 갱신, 재고 확정은 하나의 트랜잭션으로 묶는다.** 결제만 승인되고 후속 상태 갱신이 누락되는 상황을 방지한다.
...
- [ ] ➕ 확인 필요 항목 일괄 검토 (카테고리 삭제 정책, 회원당 기본배송지 unique 처리 방식)

## 9. 초기 백업 및 복구 인프라 정책
- **백업**: 매일 03:00 KST 자동 실행 (일간 31개, 월간 12개 보관)
- **복구**: 목표 RPO 24시간, RTO 4시간
- **운영**: 전용 DB 생성 후 최초 복구 테스트를 수행하며, 운영 담당자가 책임진다.
