...
#### 상품 단건 조회 (관리자용) 🔒 — ➕ 신규 추가
- **Method/URL**: `GET /api/v1/admin/products/{productId}`
- **설명**: 숨김/단종 상태를 포함해 상품 상세를 조회한다. 고객용 `GET /products/{productId}`는 판매중 상품만 노출하므로(비판매중 상태는 `404`), 관리자 화면(상품 수정)에서는 이 API가 별도로 필요하다.
- **인증**: 필요 / **권한**: 관리자
- **요청**: 없음
- **응답 요약**: 상품 상세(모든 상태 포함) + 옵션/이미지 목록
...
#### 재고 목록 조회 🔒 🔴 — ➕ 신규 추가
- **Method/URL**: `GET /api/v1/admin/inventory`
- **설명**: 옵션별 재고 현황(총재고/예약중/가용재고) 목록 조회. 재고 관리 화면(`ADM_INVENTORY_MANAGE`)의 기본 조회 API로, 기존에는 개별 조정 API만 있고 목록 조회가 누락되어 있었다.
- **인증**: 필요 / **권한**: 관리자
- **요청**: `page`, `size`, `keyword`(상품명/옵션명)
- **응답 요약**: 옵션별 재고 목록(상품명, 옵션명, 총재고, 예약중, 가용재고) + 페이징 메타
- **주요 실패 케이스**: 없음
...
- **설명**: 전체 회원 목록 조회 및 검색. 기존 API 목록에 관리자용 회원 조회 API 자체가 누락되어 있었다.
- **인증**: 필요 / **권한**: 관리자
- **요청**: `page`, `size`, `keyword`(이름/이메일), `status`
- **응답 요약**: 회원 목록(이름, 이메일, 가입일, 상태, 최근로그인) + 페이징 메타. **비밀번호 해시 등 민감정보는 응답에 포함하지 않는다**
- **주요 실패 케이스**: 없음

### 10-8. 대시보드 — ➕ 신규 추가

#### 대시보드 요약 통계 조회 🔒
- **Method/URL**: `GET /api/v1/admin/dashboard/summary`
- **설명**: 오늘 주문수/매출, 신규 회원수, 결제실패 건수 등 관리자 대시보드(`ADM_DASHBOARD`)에 필요한 요약 통계 조회
- **인증**: 필요 / **권한**: 관리자
- **요청**: 없음 (필요 시 기간 파라미터 `fromDate`,`toDate` 추가 검토)
- **응답 요약**: 요약 통계 객체(`todayOrderCount`, `todayRevenueAmount`, `newUserCount`, `paymentFailedCount` 등) + 최근 주문 목록 일부
- **주요 실패 케이스**: 없음
- **비고**: 구체적인 집계 항목/기준(예: "오늘"의 시간대 기준)은 실제 구현 전 별도 확정이 필요하다 (➕ 확인 필요)
