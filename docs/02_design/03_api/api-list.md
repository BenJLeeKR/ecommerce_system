...
- **설명**: `PAYMENT_FAILED` 상태 주문에 대해 재고를 재예약하고 새 결제 시도를 생성. 결제 시도 한도는 3회로 제한하며, 3회 누적 실패 시 더 이상 재결제를 시도할 수 없도록 차단한다.
- **인증**: 필요
- **권한**: 본인 소유 주문만
- **요청**: `paymentMethod`
...
- **응답 요약**: 수신 확인 응답
- **주요 실패 케이스**: 서명 불일치/위조 의심(`401`/`403`), **중복 콜백**은 오류가 아니라 정상 응답하되 실제 처리는 생략(복합 멱등성 정책, `domain-model.md` 8장). 역순·불일치 콜백 발생 시 단순 무시하지 않고 로그(민감정보 제외) 기록 후 운영팀 수동 대사 처리로 연계.
