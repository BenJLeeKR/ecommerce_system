...
2. **상태 조회와 외부 API의 분리 및 원문 비영속 경계**:
   - 외부 StateRepository에서의 등록 조회(비민감 식별자·해시 위주)와 실제 원격 상태/원문 조회를 엄격히 분리한다.
   - 원문 Plan, 프롬프트, 활동(Activity) 등의 데이터는 오직 Codex의 수동 검토 요청 진입점에서만 일시적인 객체로 전달되며, Git 파일, 문서, PR Description, SQLite DB, 파일 시스템, 운영 로그 등 어디에도 영구 저장되지 않는다(비영속 경계).
   - 등록된 식별자와 해시가 요청 객체 및 승인 증적과 완벽히 일치할 때만 원격 상태 조회 혹은 기존 Reader 어댑터로 정확히 1회 위임한다.
   - 조회 과정에 새로운 폴링(Polling)이나 재시도를 도입하지 않는다.
   - 동일 Contract 범위 내에서는 비민감한 보완 메시지만 허용하며, 범위나 목표 등 본질적 변경이 발생할 경우 반드시 새 Contract, 승인, 세션으로 분리해야 한다.
