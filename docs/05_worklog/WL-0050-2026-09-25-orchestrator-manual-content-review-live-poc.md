...
- **Pre-PR 진입점 추가**: `.orchestrator/src/orchestrator/manual_registered_content_review_entrypoint.py`
  ...
  - **저장소 명시적 대조 추가**: `StateRepository`를 통해 저장된 `TaskRecord`와 `ApprovalEvidence`를 직접 조회하여, 요청된 Contract 및 승인 상태와 `task_id`, `base_commit_sha`, `contract_hash`, `approved_scope_hash`, `idempotency_key`, `contract_version`, `approval_id`, ACTIVE 상태가 모두 완벽히 일치하는지 엄격히 대조합니다.
...
- **테스트 추가**: `.orchestrator/tests/test_manual_registered_content_review_entrypoint.py`
  - 저장소 증적 불일치(`TaskRecord`, `ApprovalEvidence` 불일치), 해시 불일치, 세션 등록 불일치 시나리오에 대해 각각 0회 호출을 명시적으로 검증.
...
## 4. 검증 결과
- 모든 단위 테스트 통과 (저장소 증적 대조 신규 테스트 포함).
