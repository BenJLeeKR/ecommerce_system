...
- 자동화(승인, 검토, 병합, 배포, 재시도, 재작업) 기능 배제 및 Live POC 재실행 불가 정책 명시.
...
- 승인 후 단일 Jules 브랜치 및 단일 PR 생성.
- 실제 반환된 PR 및 브랜치 정보를 기반으로 세션과의 1:1:1 사후 결속 진행.
- 미병합 시 PR 종료 및 병합 후 이슈 발생 시 merge commit revert 등의 수동 롤백 정책 적용 안내.
