...
## 설명
Orchestrator의 상태와 기록을 관리하는 Runtime SQLite DB를 안정적으로 운영하기 위한 실제 설정 및 이관 작업을 수행한다. 현재 단계에서는 `ORCHESTRATOR_JULES_STATE_DIR` 환경 변수를 통한 검증된 상태 디렉터리 경로 로더만 제공된 상태이다. 따라서 목표 경로인 `/workspace/runtime/`으로 실제 환경에 구성하고, 실제 `.env` 설정, 기존 DB 호출부 연결, DB 생성, 백업 및 복구 체계를 적용하기 위해서는 사용자의 결정과 별도 승인이 필요하다.
