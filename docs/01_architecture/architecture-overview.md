...
전용 Database 안의 구조는 **도메인별로 물리적인 Schema를 나누지 않고, 하나의 Schema(public) 안에서 테이블 이름에 도메인 접두어를 붙이는 방식**으로 결정했다. (검토 과정에서 "도메인별 Schema 분리" 안도 함께 검토했으나, 이미 전용 Database로 다른 프로젝트와 분리되어 있어 설정 복잡도를 늘릴 실익이 적다고 판단해 단순한 방식을 채택했다)

```
전용 Database (예: ecommerce_db)
└── public 스키마 (기본 스키마 1개만 사용)
    ├── user_...      (회원, 관리자 계정 관련 테이블)
    ├── catalog_...   (상품, 카테고리 관련 테이블)
    ├── order_...  🔴 (장바구니, 주문, 주문상품 관련 테이블)
    ├── payment_... 🔴 (결제 내역 관련 테이블)
    ├── inventory_... 🔴 (재고 관련 테이블)
    ├── shipping_...  (배송지, 배송상태 관련 테이블)
    └── search_...    (검색 관련 보조 테이블, MVP는 최소화)
```

**핵심 원칙**: 물리적으로 스키마가 나뉘어 있지 않은 만큼, **규칙으로 경계를 지킨다.**

- 나중에 분리하기로 한 영역(주문/결제/재고/검색)의 테이블은 **다른 도메인 접두어를 가진 테이블과 직접 조인(JOIN)하지 않는다.** 대신 ID 값만 저장해서 참조한다. (예: `order_orders` 테이블에는 상품 상세 정보를 복사하지 않고 `product_id`만 저장하고, 필요하면 카탈로그 모듈에 다시 물어본다)
- 이 규칙은 DB 자체가 강제하지 않으므로, **Claude Code가 코드 작성 시 반드시 지켜야 하는 규칙**으로 12장에 명시한다.
- 나중에 특정 도메인을 별도 Database로 분리해야 할 시점이 오면, 접두어로 묶인 테이블들만 골라서 이관하면 된다. 지금 이 방식을 선택해도 분리 자체가 불가능해지는 것은 아니며, 다만 분리 작업 시 "스키마 단위 이관"보다는 "테이블 목록을 골라내는" 한 단계가 더 필요할 뿐이다.
